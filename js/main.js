...
import * as SlidePage from './slide.js';
...
$(function () {
    ...
    allImgBox.forEach(imgBox => imgBox.addEventListener("click", e => {
        ...
        if (SlidePage.updateSelectProject(selectProject, imgBox.parentElement))
            e.stopPropagation();

        // open slide
        if (SlidePage.openSlide(slidePage))
            e.stopPropagation();

        // update slide
        let infoProject = SlidePage.getInfoProject(itemProject);
        SlidePage.insertInfoInSlide(slidePage, infoProject);
    }));
    ...
    document.addEventListener("click", e => {
        ...
        if (e.target) {
            if (selectProject)
                SlidePage.closeSlide(slidePage);
            // click link Project
    ...
    // Button Slide
    // Button close Slide
    document.querySelector("button#btnCloseSlide").addEventListener("click", () => SlidePage.closeSlide(slidePage));
    // button Pagination
    document.querySelectorAll("button.btnPagination").forEach(btn => btn.addEventListener("click", e => {
        ...
        SlidePage.updateSelectProject(selectProject, newSelectProject);

        SlidePage.insertInfoInSlide(slidePage, SlidePage.getInfoProject(newSelectProject));
    }));
    ...
    window.addEventListener('resize', () => SlidePage.slideFullPage(slidePage, header));
